$(function () {
  ...
  function changesDirectionsList() {
    const state = $(".directions__list-item:visible").length;
    if (state === 1) {
      $(".directions__list").addClass("directions__list--media");
    } else {
      $(".directions__list").removeClass("directions__list--media");
    }
  }
  ...
  var mixer = mixitup(".directions__list", {
    ...
    callbacks: {
      onMixStart: function () {
        if ($(window).width() > 700 && $(window).width() < 860) {
          mixer.configure({
            animation: {
              duration: 450,
            },
          });
        } else if ($(window).width() <= 700 || $(window).width() >= 860) {
          mixer.configure({
            animation: {
              duration: 800,
            },
  ...
  const footerLinks = document.querySelectorAll(
    ".footer__column-list [data-filter]",
  );
  const filterButtons = document.querySelectorAll(
    ".directions__filter-box button",
  );
